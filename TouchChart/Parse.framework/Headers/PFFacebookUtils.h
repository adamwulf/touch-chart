...
/*!
 Requests new Facebook permissions for the given user.  This may prompt the user to reauthorize the application.
 ...
 @param permissions The new permissions to request.
 @param block The block to execute. The block should have the following argument signature: (BOOL succeeded, NSError *error)
 */
+ (void)reauthorizeUser:(PFUser *)user withPermissions:(NSArray *)permissions block:(PFBooleanResultBlock)block;
...
/*!
 ...
 @param selector The selector that will be called when the asynchronous request is complete.
 */
+ (void)reauthorizeUser:(PFUser *)user withPermissions:(NSArray *)permissions target:(id)target selector:(SEL)selector;
