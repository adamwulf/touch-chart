...
@protocol PF_FBGraphObject;

@interface PF_FBUtility : NSObject
...
+ (id<PF_FBGraphObject>)graphObjectInArray:(NSArray*)array withSameIDAs:(id<PF_FBGraphObject>)item;
...
#define PF_FBConditionalLog(condition, desc, ...) \
do { \
    if (!(condition)) {	\
        NSString *msg = [NSString stringWithFormat:(desc), ##__VA_ARGS__]; \
        NSLog(@"PF_FBConditionalLog: %@", msg); \
    } \
} while(NO)

#define PF_FB_BASE_URL @"facebook.com"
